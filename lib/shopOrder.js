 ...
 * @param {{ product: { name: string, price: number }, quantity: number }[]} items
 */
export function buildShopCartGmailComposeHref(items) {
  const to = getShopOrderEmail();
...
/**
 * Apre il flusso email ordine: mailto su mobile, Gmail web su desktop.
 * @param {{ product: { name: string, price: number }, quantity: number }[]} items
 */
export function openShopOrderEmail(items) {
  if (typeof window === 'undefined') return;

  const mailtoHref = buildShopCartEmailHref(items);
  ...
  const gmailHref = buildShopCartGmailComposeHref(items);
