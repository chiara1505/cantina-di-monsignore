 ...
 * @param {number} quantity
 */
export function buildShopEmailParts(product, quantity) {
  return {
    to: getShopOrderEmail(),
    subject: `Ordine shop - ${product.name}`,
    body: buildShopOrderMessage(product, quantity),
  };
}
...
export function buildShopEmailHref(product, quantity) {
  const { subject, body } = buildShopEmailParts(product, quantity);
 ...
 * @param {number} quantity
 */
export function buildShopGmailHref(product, quantity) {
  const { to, subject, body } = buildShopEmailParts(product, quantity);
  const params = new URLSearchParams({
    view: 'cm',
    fs: '1',
    to,
 ...
 * @param {number} quantity
 */
export function buildShopOutlookHref(product, quantity) {
  const { to, subject, body } = buildShopEmailParts(product, quantity);
  const params = new URLSearchParams({
    to,
    subject,
