...
export function mapShopProductFromPayload(doc) {
  ...
  const legacyCarouselPath =
    typeof doc.slug === 'string' ? `/assets/images/shop/${doc.slug}.png` : legacyImagePath

  const image = resolvePayloadMediaUrl(productMedia, legacyImagePath, 'original')
  const imageCarousel = carouselMedia
    ? resolvePayloadMediaUrl(carouselMedia, legacyCarouselPath, 'original')
    : image
