...
/**
 * Prefer static `/assets/...` fallbacks when Payload media URLs point to
 * `/api/media/file/...` that may be unavailable on Vercel (Blob not synced).
 *
...
export function resolvePayloadMediaUrl(media, fallback = SHOP_FALLBACK_IMAGE, size = 'original') {
  ...
  if (
    candidate.startsWith('/api/media/file/') &&
